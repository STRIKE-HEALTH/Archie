...
namespace Archie
{
    public class iSiteReportCollector: IReportCollector
    {
        ...
        public List<exam> CollectData(Queue<string> Accessions)
        {
            logger.LogDebug("ErrorCode from iSite:" + _myiSite.GetLastErrorCode());
            ...
            try
            {
                ...
                while ((Accessions.Count>0) && _collectingData && !_disengage)
                {
                    String convertedQuery = string.Empty;
                    _accession = Accessions.Dequeue();



                    var queryWithAccession = $"IDXExamStatus IN(\"P\",\"F\",\"A\",\"R\") AND x00080050 = \"{_accession}\"";
                    //convertedQuery = queryWithDateVariables.Replace("ACCESSION", _accession);
                    string status = string.Format("Querying iSite for accession ", _accession);
                    logger.LogDebug(status);
                    // UpdateStatus(status);
                    try
                    {
                        // if (myiSite.GetLastErrorCode() == 0)
                        {
                            queryXml = _myiSite.Query(queryWithAccession, "LOOKUP", maxQueryResults);
                            logger.LogDebug($"LOOKUP {queryWithAccession}");
                            ...
                            if (!String.IsNullOrEmpty(queryXml))
                            {
                                ...
                                XDocument doc = XDocument.Parse(queryXml); //or XDocument.Load(path)
                                logger.LogDebug($"Xdocument parsed from XML");
                                string jsonText = string.Empty;
                                try
                                {
                                    logger.LogDebug($"Converting XML JSON");
                                    jsonText = JsonConvert.SerializeXNode(doc);
                                }
                                catch (Exception ex)
                                {
                                    logger.LogDebug(ex,$"Cconvert XML TO JSON failed in exception ");
                                    logger.LogDebug($"Cconvert XML TO JSON failed in exception{ex.InnerException} ");
                                }
                                //dynamic resultObjs = JsonConvert.DeserializeObject<ExpandoObject>(jsonText);
        ...
        private void GetReports(List<exam> exams)
        {
            foreach (var exam in exams)
            {
                try
                {
                    ...
                    else
                    {
                        ...
                        try
                        {
                            ...
                            logger.LogDebug($"Report Json {jsonText}");
                            
                            try
                            {
                                var reeps = JsonConvert.DeserializeObject<GetReportOutput>(jsonText);

                                exam.Reports.AddRange(reeps.AllReports.Reports.Select(x => new Report { Text = x.Text, Status = x.Status, ReportDt = x.ReportDt, Current = x.Current, PrincipalInterpreter = x.PrincipalInterpreter }));

                                //foreach ( Query.Report rep in reeps.AllReports.Reports)
                                //{
                                //    exam.Reports.Add(new Report {Text=rep.Text,Current = rep.Current,ReportDt=rep.ReportDt,Status=rep.Status,PrincipalInterpreter=rep.PrincipalInterpreter});
                                //}
                            ...
                            {
                                logger.LogDebug(e, "Exception in Deseriliaze Get Reprot Data");
                            }
                            ...
                            //        PrincipalInterpreter = rep.Value["@PrincipalInterpreter"].Value
                            //    };
                        ...
                        {
                            logger.LogError(e,"Exception in report parsing");
                        }
                ...
                catch (Exception e)
                {
                    ...
                    logger.LogError(e,"Exception in Get Reports");
                }
            }
